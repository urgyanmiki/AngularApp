...
export class CartService {
  ...
  addToCart(productname,price){
    ...
    }>("api/order/new",orderData).subscribe(response=>{
  ...
  pushToCart(productname,price){
    ...
    }>("api/order/push:"+orderid,orderData).subscribe(response=>{
    this.orderid=response.orderid
    console.log(response);
    console.log(this.orderid);
    });
  ...
  getShoppingCart(){
    ...
    console.log(orderid);
    this.http
    .get<{message: string; order: any}>("api/order/"+ orderid)
    .pipe(map((orderData) => {
  ...
  destroyOrder(){
    const orderid=this.getOrderId()
    console.log(orderid)
    this.http.delete("api/order/delete"+orderid)
    this.deleteOrderId();
  }
}
